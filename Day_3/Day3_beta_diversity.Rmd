...
nmds_data <- as.data.frame(nmds_spec_comp_bray$points)

# Add metadata (sex and Time) to the NMDS coordinates
nmds_data$Ecological.gradient <- sample_data(physeq)$Ecological.gradient
nmds_data$infection <- sample_data(physeq)$infection

# Plot the NMDS result using ggplot2
ggplot(nmds_data, aes(x = MDS1, y = MDS2)) +
    geom_point(aes(color = Ecological.gradient), size = 3, alpha = 0.5) +  # Use 'sex' here
    geom_text_repel(aes(label = infection), size = 3, color = "black") +
    ggtitle("Bray-Curtis NMDS colored according to Ecological Gradient") +  
    theme_minimal()
...
```{r}
# Extract NMDS coordinates
nmds_data <- as.data.frame(nmds_spec_comp_euclidean$points)
...
ggplot(nmds_data, aes(x = MDS1, y = MDS2)) +
    geom_point(aes(color = Ecological.gradient), size = 3, alpha = 0.5) +  # Use 'sex' here
    geom_text_repel(aes(label = infection), size = 3, color = "black") +
    ggtitle("Euclidian NMDS colored according to Ecological Gradient") +  
    theme_minimal()
```
...
```{r}
# Extract NMDS coordinates
nmds_data <- as.data.frame(nmds_spec_comp_aitchison$points)
...
ggplot(nmds_data, aes(x = MDS1, y = MDS2)) +
    geom_point(aes(color = Ecological.gradient), size = 3, alpha = 0.5) +  # Use 'sex' here
    geom_text_repel(aes(label = infection), size = 3, color = "black") +
    ggtitle("Aitchison NMDS colored according to Ecological Gradient") +  
    theme_minimal()
```
...
```{r}
clr_trans <- microbiome::transform(physeq, transform = "clr")
clr_trans  <- abundances(clr_trans) %>% as.data.frame() 
clr_trans  <- clr_trans[order(rowSums(clr_trans), decreasing = TRUE), ] %>% t()
# Calculate distance matrix
species_frac_filtered_dist_euclidean <- vegdist(clr_trans, method = "euclidean")
# Perform NMDS on distance matrix
...
```{r}
# Extract NMDS coordinates
nmds_data <- as.data.frame(nmds_spec_clr_euclidean$points)
...
```{r mia}
pseq_mia <-  mia::makeTreeSummarizedExperimentFromPhyloseq(physeq)
...
```{r}
ig <- make_network(physeq, dist.fun = "bray", max.dist = 0.2)

p <- plot_network(ig, physeq, 
